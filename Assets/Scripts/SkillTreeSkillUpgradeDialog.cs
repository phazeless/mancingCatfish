...
public class SkillTreeSkillUpgradeDialog : MonoBehaviour
{
	...
	public void Setup(Skill skill)
	{
		...
		this.TweenDialog();
		AudioManager.Instance.PlaySpecial(this.whoosh);
		...
		{
			this.upgradeCostLabel.SetText("<b>MAXED</b>");
		}
		else
		{
			this.upgradeCostLabel.SetVariableText(new string[]
			{
				this.currentSkill.CostForNextLevelUp.ToString()
			});
		}
		this.titleLabel.SetText(this.currentSkill.GetExtraInfo().TitleText);
		this.button.interactable = this.currentSkill.IsAvailableForLevelUp;
	...
	{
		this.LevelLabelHolder.localScale = Vector3.one;
		this.LevelLabelHolder.DOKill(true);
		this.LevelLabelHolder.DOPunchScale(Vector3.one * 0.3f, 0.3f, 10, 1f);
		this.levelLabel.SetVariableText(new string[]
		{
			this.currentSkill.CurrentLevel.ToString(),
			this.currentSkill.MaxLevel.ToString()
	...
	public void TryLevelUp()
	{
		...
		this.currentSkill.TryLevelUp();
		AudioManager.Instance.PlaySpecial(this.skillLvlup);
		this.UpdateUI();
	}
	...
	private void TweenKiller()
	{
		...
		this.LevelLabelHolder.DOKill(false);
	}
	...
	[SerializeField]
	private TextMeshProUGUI upgradeCostLabel;
	...
	[SerializeField]
	private Transform LevelLabelHolder;

	[SerializeField]
	private Transform edgeTransform;

	[SerializeField]
	private AudioClip skillLvlup;
